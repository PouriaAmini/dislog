// Package main implements a command-line interface for the dislog distributed
// logging system.
package main

import (
	"github.com/pouriaamini/proglog/internal/agent"
	"github.com/pouriaamini/proglog/internal/config"
...
// main is the entry point of the dislog CLI.
// It sets up the CLI app and executes it.
func main() {
	cli := &cli{}
...
// cli represents the CLI configuration.
type cli struct {
	cfg cfg
}

// cfg represents the configuration of the dislog agent.
type cfg struct {
	agent.Config
...
// setupFlags sets up the CLI flags and binds them to the viper configuration.
...
// setupConfig reads the configuration and prepares the agent’s
// configuration. Cobra calls setupConfig before running the command’s RunE
// function.
...
// run runs the cmds passed to the program
